...
1. Auto-reset order counter settings — automatically reset the order number counter daily, weekly, monthly, or yearly, with a configurable reset counter value.
2. Renumber Orders tool — reassign custom order numbers to all existing orders in sequence.
...
* Fix - Dynamic prefix and suffix placeholders are displayed in newly generated order numbers when switching from the Pro version to the Lite version.
